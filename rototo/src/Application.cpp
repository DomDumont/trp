...
int ret;
char coucou[10] = "coucou";
SDL_RWops *out = g_app->resourceManager->SaveFromFullPath("writehere.xml");
SDL_RWwrite(out,coucou,7,1);
SDL_RWclose(out);
*/
...
int CApplication::Run()
{
    ...
    while (doneCode == DONECODE_NOT_DONE) 
		...
		//scriptManager->RunScript("void OnUpdate(uint64 _delta)","L",elapsed);		
		//scriptManager->RunScript("void OnRender(uint64 _delta)","L",elapsed);
...
void CApplication::ReadSettings()
{
	...
	for(tinyxml2::XMLElement* elem = root->FirstChildElement(); elem != NULL; elem = elem->NextSiblingElement())
    {
       ...
       if (strcmp(elem->Value(),"server") == 0)
       {
        ...
        /*
         SDL_Log(elem->Attribute("name"));
         SDL_Log(elem->Attribute("x"));
         SDL_Log(elem->Attribute("y"));
         SDL_Log(elem->Attribute("width"));
         SDL_Log(elem->Attribute("height")); */
     }
}
