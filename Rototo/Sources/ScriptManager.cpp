...
void ScriptManager::Prepare()
{
	...
	if (strstr(asGetLibraryOptions(), "AS_MAX_PORTABILITY") == 0)
	{
		...
		r = engine->RegisterObjectMethod("Button", "Widget@ opImplCast()", asFUNCTION(refCastButton2Widget), asCALL_CDECL_OBJLAST); SDL_assert(r >= 0);
	...
	else
	{
		...
		r = engine->RegisterObjectMethod("Button", "Widget@ opImplCast()", WRAP_OBJ_LAST(refCastButton2Widget), asCALL_GENERIC); SDL_assert(r >= 0);
